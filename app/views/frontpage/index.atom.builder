...
atom_feed(
  ...
  feed.updated   @posts.empty? ? Time.now.utc : @posts.collect(&:published_at).max
  ...
  @rssposts.each do |post|
   feed.entry(post, :url => post_path(post, :only_path => false), :published => post.published_at, :updated => post.published_at) do |entry|
