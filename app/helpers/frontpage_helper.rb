module FrontpageHelper
  ...
  def previous_page_button
    arrow = !@posts.first.previous.empty? ? link_to_remote(image_tag('arrow_left.jpg', :class => 'arrow'), :url => frontpage_path(:post => @posts.first, :section => @section, :direction => 'previous'), :html => {:class => 'ico-prev'}) : "&nbsp;"
    content_tag :div, arrow, :id => 'leftArrow'
  end

  def next_page_button
    arrow = !@posts.last.next.empty? ? link_to_remote(image_tag('arrow_right.jpg', :class => 'arrow'), :url => frontpage_path(:post => @posts.last, :section => @section, :direction => 'next'), :html => {:class => 'ico-next'}) : "&nbsp;"
